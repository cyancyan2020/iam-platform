...
('/system/users',        'MENU', '用户管理', 0),
('/system/roles',        'MENU', '角色管理', 0),
('/system/permissions',  'MENU', '权限管理', 0);
...
-- 权限管理页面权限
SET @perm_menu_id = (SELECT id FROM permission WHERE path = '/system/permissions' AND method = 'MENU');
INSERT INTO `permission` (`path`, `method`, `name`, `parent_id`) VALUES
